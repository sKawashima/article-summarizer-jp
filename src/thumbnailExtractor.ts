...
function filterUnwantedImages(images: ImageCandidate[]): ImageCandidate[] {
  return images.filter((image) => {
    ...
    const unwantedClassPatterns = ['icon', 'logo', 'avatar', 'ad', 'advertisement', 'favicon', 'sprite', 'button'];
...
function selectBestThumbnail(images: ImageCandidate[]): string | undefined {
  ...
  if (articleImages.length > 0) {
    ...
    const largeImages = articleImages.filter(
      (img) => (img.width && img.width > 300) || (img.height && img.height > 300)
    );
  ...
  if (generalImages.length > 0) {
    const largeImages = generalImages.filter(
      (img) => (img.width && img.width > 300) || (img.height && img.height > 300)
    );
    if (largeImages.length > 0) {
      return largeImages[0].url;
